...
import ezcompress.bitstream.DefaultInputBitStream;
...
public class CompressedInputStream extends InputStream
{
   ...
   protected void readHeader() throws IOException
   {
      ...
      if (type != BITSTREAM_TYPE)
         throw new ezcompress.io.IOException("Invalid stream type", Error.ERR_INVALID_FILE);
      ...
      if (bsVersion > BITSTREAM_FORMAT_VERSION)
         throw new ezcompress.io.IOException("Invalid bitstream, cannot read this version of the stream: " + bsVersion,
      ...
      if ((this.blockSize < MIN_BITSTREAM_BLOCK_SIZE) || (this.blockSize > MAX_BITSTREAM_BLOCK_SIZE))
         throw new ezcompress.io.IOException("Invalid bitstream, incorrect block size: " + this.blockSize,
      ...
      if (this.listeners.size() > 0)
      {
         ...
         {
            throw new ezcompress.io.IOException("Invalid bitstream, unknown entropy codec type: "+
         ...
         {
            throw new ezcompress.io.IOException("Invalid bitstream, unknown transform type: "+
         ...
         while (true)
         {
            ...
            if (nbTasks == 1)
            {
               ...
               if (status.decoded > this.blockSize)
                  throw new ezcompress.io.IOException("Invalid data", Error.ERR_PROCESS_BLOCK);               
            }
            else
            {
