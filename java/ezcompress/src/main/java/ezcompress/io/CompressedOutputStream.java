...
import ezcompress.BitStreamException;
import ezcompress.EntropyEncoder;
import ezcompress.Error;
import ezcompress.Event;
import ezcompress.Global;
import ezcompress.Listener;
import ezcompress.OutputBitStream;
import ezcompress.SliceByteArray;
import ezcompress.bitstream.DefaultOutputBitStream;
import ezcompress.entropy.EntropyCodecFactory;
import ezcompress.entropy.EntropyUtils;
import ezcompress.transform.Sequence;
import ezcompress.transform.TransformFactory;
import ezcompress.util.hash.XXHash32;
...
public class CompressedOutputStream extends OutputStream {
	...
	public CompressedOutputStream(OutputBitStream obs, Map<String, Object> ctx) {
		...
		if (bSize > MAX_BITSTREAM_BLOCK_SIZE)
			throw new IllegalArgumentException(
					"The block size must be at most " + (MAX_BITSTREAM_BLOCK_SIZE >> 20) + " MB");
	...
	protected void writeHeader() throws IOException {
		if (this.obs.writeBits(BITSTREAM_TYPE, 32) != 32)
			throw new ezcompress.io.IOException("Cannot write bitstream type to header", Error.ERR_WRITE_FILE);

		if (this.obs.writeBits(BITSTREAM_FORMAT_VERSION, 4) != 4)
			throw new ezcompress.io.IOException("Cannot write bitstream version to header", Error.ERR_WRITE_FILE);

		if (this.obs.writeBits((this.hasher != null) ? 1 : 0, 1) != 1)
			throw new ezcompress.io.IOException("Cannot write checksum to header", Error.ERR_WRITE_FILE);

		if (this.obs.writeBits(this.entropyType, 5) != 5)
			throw new ezcompress.io.IOException("Cannot write entropy type to header", Error.ERR_WRITE_FILE);

		if (this.obs.writeBits(this.transformType, 48) != 48)
			throw new ezcompress.io.IOException("Cannot write transform types to header", Error.ERR_WRITE_FILE);

		if (this.obs.writeBits(this.blockSize >>> 4, 28) != 28)
			throw new ezcompress.io.IOException("Cannot write block size to header", Error.ERR_WRITE_FILE);

		if (this.obs.writeBits(this.nbInputBlocks & (MAX_CONCURRENCY - 1), 6) != 6)
			throw new ezcompress.io.IOException("Cannot write number of blocks to header", Error.ERR_WRITE_FILE);

		if (this.obs.writeBits(0L, 4) != 4)
			throw new ezcompress.io.IOException("Cannot write reserved bits to header", Error.ERR_WRITE_FILE);
	}
	...
	public void write(int b) throws IOException {
		try {
			if (this.buffers[this.bufferId].index >= this.bufferThreshold) {
				...
				if (this.bufferId + 1 < Math.min(this.nbInputBlocks, this.jobs)) {
					...
					if (this.closed.get() == true)
						throw new ezcompress.io.IOException("Stream closed", Error.ERR_WRITE_FILE);
			...
			throw new ezcompress.io.IOException(e.getMessage(), Error.ERR_READ_FILE);
		} catch (ezcompress.io.IOException e) {
			throw e;
		} catch (Exception e) {
			throw new ezcompress.io.IOException(e.getMessage(), Error.ERR_UNKNOWN);
		}
	}
	...
	public void close() throws IOException {
		...
		try {
			...
			throw new ezcompress.io.IOException(e.getMessage(), e.getErrorCode());
		}
		...
		try {
			...
			for (int taskId = 0; taskId < nbTasks; taskId++) {
				...
				Callable<Status> task = new EncodingTask(this.buffers[taskId], this.buffers[this.jobs + taskId],
						...
						this.hasher, this.blockId, blockListeners, map);
				tasks.add(task);
			}
			...
			if (tasks.size() == 1) {
				...
				if (status.error != 0)
					throw new ezcompress.io.IOException(status.msg, status.error);
			} else {
			...
			int errorCode = (e instanceof BitStreamException) ? ((BitStreamException) e).getErrorCode()
					: Error.ERR_UNKNOWN;
			throw new ezcompress.io.IOException(e.getMessage(), errorCode);
		}
	}
	...
	static class EncodingTask implements Callable<Status> {
		...
		EncodingTask(SliceByteArray iBuffer, SliceByteArray oBuffer, int length, long transformType, int entropyType,
				int blockId, OutputBitStream obs, XXHash32 hasher, AtomicInteger processedBlockId, Listener[] listeners,
		...
		private Status encodeBlock(SliceByteArray data, SliceByteArray buffer, int blockLength, long blockTransformType,
			...
			try {
				...
				if (this.listeners.length > 0) {
					...
					Event evt = new Event(Event.Type.AFTER_ENTROPY, currentBlockId, (written + 7) >> 3, checksum,
							this.hasher != null);
